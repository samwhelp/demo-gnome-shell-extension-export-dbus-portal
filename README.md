

# demo-gnome-shell-extension-export-dbus-portal




## Home




## Subject

* [Discussion](#discussion)
* [Reference](#reference)
* [Toggle Tool](#toggle-tool)
* [Keyword](#keyword)
* [Link](#link)




## Discussion

* [#309 - Where is the taskbar Activities button?](https://github.com/Anduin2017/AnduinOS/discussions/309#discussioncomment-15678560)




## Reference

* [dash-to-plank](https://github.com/hardpixel/dash-to-plank)
* [demo-toggle-overview-via-dbus](https://github.com/samwhelp/demo-for-anduinos-discussion/tree/main/demo/gnome-shell/demo-toggle-overview-via-dbus)
* https://gjs.guide/extensions/development/creating.html
* https://gjs.guide/guides/gio/dbus.html




## Toggle Tool

| Toggle Tool |
| ----------- |
| [~/.local/share/applications/gnome-shell-toggle-overview.desktop](asset/applications/gnome-shell-toggle-show-activities.desktop) |
| [~/.local/share/applications/gnome-shell-toggle-show-applications.desktop](asset/applications/gnome-shell-toggle-show-applications.desktop) |




## Keyword

* Search: [gnome shell application overview command line](https://www.google.com/search?q=gnome+shell+application+overview+command+line)
* Search: [org.gnome.Shell.Eval](https://www.google.com/search?q=org.gnome.Shell.Eval)




## Link

* https://askubuntu.com/questions/1201849/invoking-gnome-show-applications-from-command-line
* https://askubuntu.com/questions/1412130/dbus-calls-to-gnome-shell-dont-work-under-ubuntu-22-04
* https://github.com/linushdot/unsafe-mode-menu
* Ubuntu Desktop Guide / [Visual overview of GNOME](https://help.ubuntu.com/stable/ubuntu-help/shell-introduction.html.en)
* [Projects/GnomeShell/CheatSheet](https://wiki.gnome.org/Projects(2f)GnomeShell(2f)CheatSheet.html)
* https://gjs.guide/
* https://gjs-docs.gnome.org/
* https://gjs.guide/extensions/
* https://gjs.guide/extensions/overview/architecture.html
* https://gjs.guide/extensions/development/creating.html
* https://github.com/GNOME/gnome-shell
* https://gitlab.gnome.org/GNOME/gnome-shell
* https://github.com/GNOME/gnome-shell/blob/main/js/ui/overview.js
* https://gitlab.gnome.org/GNOME/gnome-shell/-/blob/main/js/ui/overview.js
* https://github.com/home-sweet-gnome/dash-to-panel
* https://gitlab.com/arcmenu/ArcMenu
